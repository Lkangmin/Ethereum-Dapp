...
const contract_address = "0x2707bB52b722f6D9c0834D23E84Ac0abb8f5Dfa8"; // 따옴표 안에 주소값 복사 붙여넣기
...
const _shareRoom = async (name, location, price) => {
  ...
  let contract = await getRoomShareContract();
...
const _getMyRents = async () => {
  // 내가 대여한 방 리스트를 불러온다.
  let contract  = await getRoomShareContract();
  let myRents = await contract.methods.getMyRents().call({from: user, gas: 3000000});
...
const _getAllRooms = async () => {
  ...
  let rooms = [];
  let contract = await getRoomShareContract();
  let roomId = await contract.methods.getRoomId().call();
  
  for (let i = 0; i < roomId; i++){
    let tempRoom = await contract.methods.roomId2room(i).call();
    rooms.push(tempRoom);
  }
...
const _rentRoom = async (roomId, currentYear, checkInDate, checkOutDate, price) => {
  ...
  // 화면을 업데이트 한다.
  let contract = await getRoomShareContract();
  await contract.methods.rentRoom(roomId, currentYear, checkInDate, checkOutDate).send({from: user, gas:30000000, value: price * Math.pow(10, 15)})
    .then(() => alert("대여 성공!"))
    .catch(err => {
...
const _recommendDate = async (roomId, checkInDate, checkOutDate) => {
  ...
  let contract = await getRoomShareContract();
  let rentDate = await contract.methods.recommendDate(roomId, checkInDate, checkOutDate).call({from: user});
  let start = dateFromDay(currentYear, rentDate[0]).toDateString();
  let end = dateFromDay(currentYear, rentDate[1]).toDateString();

  alert("기존에 대여된 날짜: " + start + " ~ " + end);
}
...
const getRoomRentHistory = async () => {
  ...
  try {
    let tempId = returnOptionsJSON().id;
    history = await contract.methods.getRoomRentHistory(tempId).call({from: user});
  }
  catch (e) {
    console.error(e);
  }
...
const markRoomAsInactive = async () => {
  ...
  let contract = await getRoomShareContract();
  await contract.methods.markRoomAsInactive(roomIddom.value).send({from: user, gas: 3000000});
  _updateRooms();
  alert("방 비활성화 완료!");
}
...
const intializeRoomShare = async () => {
  ...
  let contract = await getRoomShareContract();
  await contract.methods.initializeRoomShare(roomIddom.value).send({from: user, gas:3000000});
  _updateRooms();
  _updateRents();
  alert("방 대여 일정 초기화 완료!");
}
